...
def run_pipeline():
    ...
    for _ in tqdm(range(NUM_SAMPLES)):
        prune_rate = np.random.uniform(0.1, 0.2)
        
        mask_enc = get_head_mask(n_layers, n_heads, pruning_rate=prune_rate)
        mask_dec = get_head_mask(n_layers, n_heads, pruning_rate=prune_rate)
        ...
        try:
            ...
            bd_scores_collected.append(bd_score)
    ...
    surrogate_util = SurrogateModel(total_heads)
    
    surrogate_util.net = nn.Sequential(
            nn.Linear(total_heads, 256), nn.ReLU(),
            nn.Linear(256, 128), nn.ReLU(),
            nn.Linear(128, 1) 
    )
    surrogate_util = surrogate_util.to(device)
